  ...
  {theme === 'dark' ? <Sun size={20} className="text-yellow-500" /> : <Moon size={20} />}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 transition-colors">
        <div className="flex items-center">
          ...
          <div className="mr-5">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white transition-colors">{getGreeting()}</h2>
            <p className="text-gray-600 dark:text-gray-300 transition-colors">
              {dashboardData.userProfile.name}
              {dashboardData.userProfile.birthDate && `, ${calculateAge(dashboardData.userProfile.birthDate)} ${t('profile.age')}`}
            </p>
            
            {/* Health Record ID */}
            <div className="mt-2 flex items-center">
              <span className="text-xs text-gray-500 dark:text-gray-400 ml-1 transition-colors">{t('profile.healthRecordId')}:</span>
              <span className="text-sm font-medium text-gray-800 dark:text-gray-200 transition-colors">{dashboardData.userProfile.healthRecordId}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {dashboardData.sections.map(section => (
        <div key={section.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden transition-colors">
          <div className={`bg-gradient-to-r from-${section.color}-50 to-white dark:from-${section.color}-900/20 dark:to-gray-800 p-4 border-b border-gray-100 dark:border-gray-700 transition-colors`}>
            <div className="flex items-center">
              <div className={`p-2 bg-${section.color}-100 dark:bg-${section.color}-900/30 rounded-full transition-colors`}>
